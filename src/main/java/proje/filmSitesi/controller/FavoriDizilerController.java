...
public class FavoriDizilerController {
	...
	public ResponseEntity<String> removeFavoriteDizi(@RequestBody RemoveFavoriDiziRequest removeKullaniciDiziRequest ) {
	    ...
	    Long kullaniciId = removeKullaniciDiziRequest.getKullaniciId();
	    Long diziId = removeKullaniciDiziRequest.getDiziId();
	    	
	    favoriDizilerDao.removeFavoriteDizi(kullaniciId, diziId);
	    	
	    return ResponseEntity.ok("Dizi favorilerinizden çıkarıldı");
	        
	    }
	
    @GetMapping("/getfavoridiziler/{kullaniciId}")
    public List<GeKullaniciFavoriteResponseDizi> getFavorilerByKullanici(@PathVariable Long kullaniciId) {

        return favoriDizilerDao.getFavorilerByKullanici(kullaniciId);
